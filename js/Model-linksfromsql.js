...
var nodeDataSqlString, linkDataSqlString, allDataSqlString;
var indicatorsFromLoaded = [];
var linkIndex;
var impEditor;
var projectName;
var descriptionFlag = false;
var parentGroupResults = [],
  parentGroupResultsSubs = [];
var data;
...
function checkContainer() {
  if ($("#d_tab").is(":visible")) {
    ...
    simulateClick(document.getElementById("LoadImplementation"), "click");
    simulateClick(document.getElementById("refreshed"), "click");
  } else {
...
$("#get-node").click(function () {
  // console.log(allDataSqlString);
});

//js to retrieve checked boxes and add to array

window.onload = function () {
  document.getElementById("viewLoaded").style.display = "none";
  simulateClick(document.getElementById("search"), "click");
  //
  ...
  $.each(allImplementationDataUF, function (i, el) {
    if ($.inArray(el, allImplementationData) === -1)
      allImplementationData.push(el);
  });
  console.log("AllImplementationData", allImplementationData);
  $.each(impObjKeys, function (i, el) {
    if ($.inArray(el, impObjKeys) === -1) impObjKeys.push(el);
  });
  $.each(impObjValues, function (i, el) {
    if ($.inArray(el, impObjValues) === -1) impObjValues.push(el);
  });
  $.each(projObjKeys, function (i, el) {
    if ($.inArray(el, projObjKeys) === -1) projObjKeys.push(el);
  });
  $.each(projObjKeys, function (i, el) {
    if ($.inArray(el, projObjKeys) === -1) projObjKeys.push(el);
  });
  for (var i = 0; i < impObjValues.length; i++) {
    implementationObject[impObjKeys[i]] = impObjValues[i];
  }
  for (var i = 0; i < projObjValues.length; i++) {
    projectObject[projObjKeys[i]] = projObjValues[i];
  }

  pfgsPerImplementation();

  document.getElementById("Filter-All").onclick = function () {
    myBarChart.data.datasets[0].data = noOfIndicators;
    myBarChart.update();
  };
  document.getElementById("Filter-Tourism").onclick = function () {
    myBarChart.data.datasets[0].data = noOfIndicators;
    myBarChart.update();
  };
  document.getElementById("Filter-Derry").onclick = function () {
    $.each(allImplementationDataUF, function (i, el) {
      if ($.inArray(el, allImplementationData) === -1)
        allImplementationData.push(el);
    });
    ...
    filterLocation("Derry, Londonderry, UK");
    console.log(noOfIndicatorsLocation);

    myBarChart.data.datasets[0].data = noOfIndicatorsLocation;
    myBarChart.update();
  };
  document.getElementById("Filter-Belfast").onclick = function () {
    $.each(allImplementationDataUF, function (i, el) {
      if ($.inArray(el, allImplementationData) === -1)
        allImplementationData.push(el);
    });
    ...
    filterLocation("Westlink, Belfast, UK");
...
var allParentGroups = ["UN", "PFG", "Making Life better"];
var groups = [];
...
function getNode() {
  ...
  $.ajax({
    ...
    type: "GET",
    url: "php/sql/getNode.php",
    dataType: "text", //expect html to be returned
    success: function (response) {
      nodeDataSqlString = response;
      console.log(allDataSqlString);
      allDataSqlString =
        allDataSqlString.slice(0, 11) +
        nodeDataSqlString.slice(0, -1) +
        allDataSqlString.slice(11);
      data = JSON.parse(allDataSqlString);
    },
  });
  $.ajax({
    //create an ajax request to display.php
    type: "GET",
    url: "php/sql/getLink.php",
    dataType: "text", //expect html to be returned
    success: function (response) {
      linkDataSqlString = response;

      allDataSqlString =
        '{"nodes": [], "links": [' + linkDataSqlString.slice(0, -1) + "] }";
      //alert(response);
    },
  });

  // console.log(newAllDataString);
...
$(document).ready(function () {
  /* $("#results-parent-sub").append(
    ' id="resultTest0"href="#menupos1" class="list-group-item collapsed collapser">Parent ItemNEW<span class="float-right"><i class="fa"></i></span></li><div class="collapse  sub-menu-bg" id="menupos23"> <li class="list-group-item sub-item">New Append</li> </div>'
  );
  $("#menupos23").append(
    '  href="#" class="list-group-item sub-item">New</li>'
  );*/
  /*var press = jQuery.Event("keyup");
  press.ctrlKey = false;
  press.which = 40;
  $("#search").typetype("UN Sustainable Dev");
  // $("#search").backspace(2);
  $("#search").trigger(press);
  $("#search").trigger($.Event("keydown", { keyCode: 40 }));
  $("#search").typetype(" ");
  $("#search").backspace(20);*/
  ...
  $("#search").on("keyup", function () {
    ...
    /* for (var i = 0; i < allParentGroups.length; i++) {
      $("#resultTest" + i).text(allParentGroups[i]);
    }*/
    parentGroupResults = [];
    parentGroupResultsSubs = [];
    $("#results-parent-sub").empty();
    //$.getJSON(newAllData, function (data) {
    $.each(data.nodes, function (key, value) {
      if (
        ...
        /*  if (!parentGroupResults.includes(value.parentGroup)) {
          parentGroupResults.push(value.parentGroup);
        }
        if (
          !parentGroupResultsSubs.includes(
            value.parentGroup + "***" + value.id && no != null
          )
        ) {
          parentGroupResultsSubs.push(
            value.parentGroup +
              "***" +
              value.group +
              " " +
        ...
        //if (!parentGroupResults.some((item) => item === whatever)) {}

        /* $("#results-parent-sub").append(
            '<li id="resultTest0"href="#menupos1" class="list-group-item collapsed collapser">' +
              key +
              '<span class="float-right"> <i class="fa"></i></span></li>'
          );*/
        if (no != null) {
          $("#result").append(
    ...
    /* for (let ele of parentGroupResults) {
      $("#menupos2").append(
        '<li href="#" class="list-group-item sub-item">' + ele + "</li>"
      );

      $("#results-parent-sub").append(
        '<li id="resultTest' +
          ele.replaceAll(" ", "") +
          '"href="#menupos1" class="list-group-item collapsed collapser">' +
          ele +
          '<span class="float-right"> <i class="fa"></i></span></li>'
      );
      for (let sub of parentGroupResultsSubs) {
        if (
          $("#resultTest" + ele.replaceAll(" ", "") + "") &&
          sub.includes(ele)
        ) {
          $("#resultTest" + ele.replaceAll(" ", "") + "").append(
            '<li href="#" class="list-group-item sub-item">' +
              sub.replace(ele + "***", "") +
              "</li>"
          );
        }
      }
      if ($("#resultTest" + ele.replaceAll(" ", "") + "")) {
          $("#resultTest" + ele.replaceAll(" ", "") + "").append(
            ' <li class="list-group-item sub-item">Item</li>          <li href="#" class="list-group-item sub-item">SubItem</li>          <li href="#" class="list-group-item sub-item">SubItem</li>        <li href="#" class="list-group-item sub-item">SubItem</li>        <li href="#" class="list-group-item sub-item">SubItem</li>     '
          );
        }
    }
    // console.log("SUBS", parentGroupResultsSubs);*/
    ...
    document.getElementById("deleteChart").onclick = function () {
      chartOverall.destroy();
    };
    $(".etab")
      .unbind()
      .click(function () {
        document.getElementById("single-project-insights").style.display =
          "block";
        document.getElementById("desc-svg-div").style.display = "none";
        document.getElementById("desc-svg").style.display = "none";
        document.getElementById("center-svg").style.display = "none";
    ...
    /* document.getElementById("save-project2").onclick = function () {
        simulateClick(document.getElementById("save-project"), "click");
      };*/
  ...
  document.getElementById("viewLoaded").onclick = function () {
    ...
    for (let node of dataToTest.nodes) {
      if (
        indicatorsFromLoaded.indexOf(node.id + " - " + node.description) === -1
      ) {
        indicatorsFromLoaded.push(node.id + " - " + node.description);
      }
    }

    //console.log("inddata", indicatorData);
    indicatorsFromLoaded.sort();
    console.log("indfromloaded", indicatorsFromLoaded);
    console.log("indData", indicatorData);
    // indicatorsFromLoaded = [];
    //indicatorData = indicatorsFromLoaded;
    projectName = allNames[modelRowIndex];
  ...
  function create(pass) {
    ...
    var nodeMenu = [
      {
        ...
        action: function (elm, d, i) {
          undo(modelStates, pass);
        },
      },
      {
        title: "Delete",
        action: function (elm, d, i) {
          linkIndex = pass.links.filter((d) => {
          ...
          /*  d3.selectAll("path") //.select(".links")
                  .filter(function (d, i) {
                    for (let lInd of linkIndex) {
                      console.log(lInd.source.id);
                      console.log(lInd.target.id);
                      return id === lInd.source.id || id === lInd.target.id;
                    }
                  })
                  .style("stroke", "white");*/

          modelStates.push(index);
        },
      },
    ];
    ...
    var bgMenu = [
      ...
      },
      {
        title: "Add",
        action: function (elm, d, i) {
          simulateClick(document.getElementById("AddModal"), "click");
        },
      },
    ];

    document.getElementById("svg-undo").onclick = function () {
      undo(modelStates, pass);
    };

    document.getElementById("svg-add").onclick = function () {
      simulateClick(document.getElementById("AddModal"), "click");
    };

    document.getElementById("add-Modal-Submit").onclick = function () {
      for (let i of passAdd.nodes) {
        if (!pass.nodes.some((item) => item === i)) {
          pass.nodes.push(i);
        }
      }
      create(pass);
    ...
    pass.nodes = getUnique(pass.nodes);

    addParents(pass.nodes, json.nodes);
    addPFGParents(pass, json);
    ...
    } else if (countParentGroups(pass.nodes) == 10) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 11) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 12) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 13) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 14) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 15) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 16) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 17) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    } else if (countParentGroups(pass.nodes) == 18) {
      regionArray = [
        [Ox, Oy],
        [Ox, h / 4],
        [w / 2, h / 3],
    ...
    //  create(foyleOverall);
    /* document.getElementById("model-svg-div").style.backgroundColor = "gray";
      document.getElementById("model-svg-div").style.opacity = "0.4";
      document.getElementById("blackout").style.display = "block";*/
    //   typeText($("#search"), "text", 500, 0);
    ...
    g.append("rect")
      ...
      .on("contextmenu", d3.contextMenu(bgMenu));
...
*/
    ...
    node
      ...
      /*.on(
              ...
              function (d, i) {
                //document.getElementById("svg-undo").style.display = "block";
                var index = i;
      ...
      .on("mouseover", function (d) {
        if (
          ...
          var boxX =
              (d.x + bbox.width + (d.type == "Parent" ? 60 : 30)) * zoomLevel +
              panX,
            boxY =
              (d.y + bbox.height + (d.type == "Parent" ? 60 : 30)) * zoomLevel +
              panY,
            ...
            topOfBox =
              (d.y - bbox.height - (d.type == "Parent" ? 60 : 30)) * zoomLevel +
              panY,
            bottomOfBox = d.y + bbox.height + (d.type == "Parent" ? 60 : 30),
    ...
    function remove(n, i) {
      ...
      modelStatesLinks.push(linkIndex);
      // console.log("linkindex", modelStatesLinks);
      /*
            d3.selectAll("path").style("stroke", function (d) {
              for (let selected of linkIndex) {
                if (
                  d.source.id == selected.source.id ||
                  d.target.id == selected.target.id ||
                  d.target.id == selected.source.id ||
                  d.source.id == selected.target.id
                ) {
                  return "none";
                } else {
                  if (d.source.colour) {
    ...
    function hidePath(selectedPaths, allPaths) {
      for (let x of selectedPaths) {
        for (let path of allPaths) {
          if (path.id.includes(x.source) || path.id.includes(x.target)) {
            return "none";
          }
        }
      }
    ...
    for (let node of pass.nodes) {
      ...
      indicatorData.push(indicators);
      //  sortArray(indicatorData);//COMEBACKTOTHIS
      //console.log(indicatorData);
      //console.log(indicatorData);
    ...
    d3.select("#download-png").on("click", function () {
      var undo = document.getElementById("svg-undo");
      var add = document.getElementById("svg-add");
      var div = document.getElementById("model-svg-div");
      var container = document.getElementById("center-svg");
      undo.remove();
      add.remove();

      //Save as image function

      var html = (descriptionFlag ? svgDescription : svg)
        .attr("version", 1.1)
      ...
      div.insertBefore(undo, container);
      div.insertBefore(add, container);
    });
  ...
  function groupToColor(d) {
    for (var i = 0; i < d.length; i++) {
      if (d[i]["parentGroup"] == "UN Sustainable Development Goals") {
        ...
        d[i]["colour"] = "CadetBlue";
      } else if (d[i]["parentGroup"] == "Culture 2030 Indicators") {
        d[i]["colour"] = "#285c00";
      } else if (d[i]["parentGroup"] == "Belfast City Council Public Realm") {
        ...
        d[i]["parentGroup"] ==
        "Derry City & Strabane Districts Inclusive Strategic Growth Plan"
      ) {
        d[i]["colour"] = "LightCoral";
      } else if (
        d[i]["parentGroup"] == "Protect Life 2 - Suicide Prevention Strategy"
      ) {
...
(originalPass);
  ...
  function showGroupFoyle(data, pass) {
    var pg1 = "UN Sustainable Development Goals",
      pg2 = "Derry City And Strabane Districts Inclusive Strategic Growth Plan",
  ...
  //addPFGParents
  function addPFGParents(pass, data) {
    var tempIDs = [];
    for (var i = 0; i < pass.nodes.length; i++) {
      for (var j = 0; j < data.links.length; j++) {
        if (
          pass.nodes[i].id == data.links[j].source &&
          pass.nodes[i].parentGroup == "Programme for Government" &&
          pass.nodes[i].type == "Child"
        ) {
          tempIDs.push(data.links[j].target);
        }
      }
    }
    console.log("temps", tempIDs);
    for (var i = 0; i < tempIDs.length; i++) {
      for (var j = 0; j < data.nodes.length; j++) {
        // console.log(data.nodes[j].id);
        if (
          tempIDs[i] == data.nodes[j].id &&
          data.nodes[j].parentGroup == "Programme for Government"
        ) {
          // if (!checkPassHasNode(pass.nodes, data[j].id)) {
          pass.nodes.push(data.nodes[j]);
          // }
        }
      }
    }
  ...
  //undo
  function undo(arr, pass) {
    var last = arr[arr.length - 1];
    console.log(arr);
    modelStatesIndex > 0 ? modelStatesIndex-- : (modelStatesIndex = 0);
    d3.select(".nodes")
      .selectAll("circle")
      .filter(function (d, i) {
        return i === last;
      ...
      .style("fill", function (d) {
        pass.nodes.push(d);
        return d.type == "Parent" ? "White" : d.colour;
      })
    ...
    d3.select(".nodes")
      ...
      .style("fill", function (d) {
        return d.type == "Parent" ? d.colour : "White";
      });

    arr.pop();
    console.log("NODES", pass.nodes);

    //  d3.selectAll("path") //.select(".links")
    /*   .filter(function (d, i) {
                    for (let lInd of linkIndex) {
                      return (
                        d.source.id === lInd.source.id ||
                        d.source.id === lInd.target.id ||
                        d.target.id === lInd.source.id ||
                        d.target.id === lInd.target.id
                      );
                    }
                  })*/
    //  .style("stroke", "red");

    /* for (let linkInd of modelStatesLinks[modelStatesLinks.length - 1]) {
          pass.links.push(linkInd);
        }*/
  }
        ...
        for (var k = 0; k < allData.links.length; k++) {
          ...
          if (
            ...
            for (var l = 0; l < jsonData.links.length; l++) {
              var contains = false;
              if (
                JSON.stringify(passLinks[l]) != JSON.stringify(linkToPush) &&
  ...
  function boolArrayToStringArray(arr, obj) {
    ...
    for (var i = 0; i < arr.length; i++) {
      if (arr[i]) {
        ...
        console.log(getKeyByValue(obj, obj[Object.keys(obj)[i]]).toLowerCase());
...
function fillInAddress() {
  ...
  console.log(place);
  const lng = (place.geometry.viewport.Sa.i + place.geometry.viewport.Sa.j) / 2;
  const lat = (place.geometry.viewport.Wa.i + place.geometry.viewport.Wa.j) / 2;

  $("#autocomplete_longitude").val(lng);
  $("#autocomplete_latitude").val(lat);
