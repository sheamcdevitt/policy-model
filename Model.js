...
$(document).ready(function () {
  ...
  $('#search').on('change paste keyup', function () {
    ...
    $.getJSON('nodes.json', function (data) {
      ...
      function create(pass) {
        ...
        d3.json("Data.json", function (error, json) {
          ...
          var simulation = d3.forceSimulation()
            ...
            .force('cluster', d3.forceCluster()
              .centers(function (d) {
                ...
                if (d.type == "Child") { return clusters[d.clusterIndex]; }
              })
              .strength(1)
          ...
          var path = svg.append("g").selectAll("path")
            ...
            })*///!Keeping here in case I come back to it
            //Mouseover functions
            //TODO brainstorm ideas with team if they need mouse over function or what that would show
      ...
      function getRadius(d) { return d.radius; }
      ...
      function legend(d) {
        ...
        for (var i = 0; i < obj.length; i++) {
          ...
          svg.append("circle").attr("cx", 25).attr("cy", y).attr("r", 15).style("fill", "White").style("stroke-width", 5).style("stroke", getColourByParentGroup(d, obj[i]));
        }
      }
      ...
      function showOnly(pass, pG) {
        temp = { "nodes": [], "links": [] };
        for (var i = 0; i < pass.nodes.length; i++) {
          if (pass.nodes[i].parentGroup == pG) { temp.nodes.push(pass.nodes[i]); }
        }
        console.log(temp);
        return temp;
      }
